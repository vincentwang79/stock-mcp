...
- 非技术用户可以只说“今天怎么样”“为什么入选”“现在确认了吗”或“帮我记一下”，
  无需记忆工具名、候选 ID、幂等键或哈希；ChatGPT 依据工具元数据完成路由。
- 未指定日期的“今天/最近”请求使用无参数 `get_latest_daily_review`，由服务端选择最新有记录
  的交易日，并保留观察、降级和失败状态，不能由模型猜测日期或退回旧成功日报。
...
- 服务器发现元数据把产品描述为面向非技术用户的“家庭 A 股盘后助手”，并声明自然语言
  交互、证据边界、写入确认和禁止交易等整体指令。
- 每个工具公开面向用户目标的中文标题、以 `Use this when` 开头的触发说明、明确的
  `Do not use` 禁止场景，以及每个输入参数的白话说明和示例。
- 所有写工具的元数据都要求用户明确提出并确认；工具注解仍是服务端安全边界，不能只依赖
  ChatGPT 提示词。
...
- 固定直接、间接和负向自然语言黄金提示；本地测试验证提示标签和元数据契约，ChatGPT
  Developer Mode 实测精确率/召回率作为部署后的外部门禁。
